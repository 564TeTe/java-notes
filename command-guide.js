...
function renderLearningResources() {
    ...
    const navigation = `<div class="learning-resource-tabs${state.resource === 'commands' ? ' showing-commands' : ''}" aria-label="学习资源类型"><button type="button" data-guide-resource="commands" aria-pressed="${state.resource === 'commands'}">命令指南</button><button type="button" data-guide-resource="websites" aria-pressed="${state.resource === 'websites'}">常用网站</button></div>`;
    ...
    return navigation + `<section class="command-guide" aria-label="常见命令指南"><div class="guide-toolbar"><div class="guide-modes" aria-label="阅读方式">${[['core','核心 40 条'],['manual','分类手册'],['scenarios','排查场景']].map(([key, title]) => `<button type="button" data-guide-mode="${key}" aria-pressed="${state.mode === key}">${title}</button>`).join('')}<button type="button" class="guide-mobile-websites" data-guide-resource="websites">网站 ↗</button></div><label class="guide-search"><span class="sr-only">搜索当前阅读分类</span><input type="search" id="guideSearch" placeholder="搜索命令、用途或环境" value="${commandEsc(state.search)}"></label><details class="guide-help"><summary>说明</summary><div class="guide-help-content"><p>点击命令即可复制。示例中的 PID、路径和地址，使用前请替换。</p><button type="button" data-guide-basics>环境与符号说明</button><a href="./assets/guides/core-40.md" download>下载核心 40 条</a><a href="./assets/guides/java-backend-commands.md" download>下载完整手册</a></div></details></div><div id="guideResults">${commandResults()}</div><span class="guide-copy-status" id="guideCopyStatus" role="status" aria-live="polite"></span></section>`;
}
...
document.addEventListener('click', async event => {
    ...
    if ('guideResource' in attr) {
        ...
        const target = [...document.querySelectorAll(`[data-guide-resource="${commandGuideState.resource}"]`)].find(el => el.getClientRects().length) || document.querySelector(`[data-guide-mode="${commandGuideState.mode}"]`);
