...
- 自定义题库题目使用 custom-bank-UUID 独立ID与 custom-bank-questions 存储；可新增、编辑、参与题库筛选/抽查，复制到笔记时才进入个人来源。快照新增 customBankQuestions；旧快照缺字段保留当前自定义题，导入先校验再修改已有状态。
- 页容量支持10/20/50/100且在设备上记忆；列表顶部/底部均有首页、上一页、下一页、末页及输入跳转（支持Enter）。改变容量重置第一页；分类/搜索重新计算页数，目录与当前分页一致。
- Node 20项通过；Playwright通过新增/编辑/重载、四种容量、末页、上下跳转、Enter、筛选与目录、320px布局验证。
