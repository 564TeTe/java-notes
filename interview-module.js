...
function renderInterviewWorkspace() {
    ...
    return `<section class="interview-workspace interview-company-layout"><aside class="interview-company-nav" aria-label="公司导航"><div class="interview-nav-title"><h2>公司目录</h2></div><button data-interview="company" data-company="all" class="${interviewCompany === 'all' && !journal ? 'active' : ''}" aria-current="${interviewCompany === 'all' && !journal ? 'page' : 'false'}"><span>全部公司</span><small>${getCompanyQuestions('all').length}</small></button>${companies.map(c => `<button data-interview="company" data-company="${studyEsc(c)}" class="${c === interviewCompany && !journal ? 'active' : ''}" aria-current="${c === interviewCompany && !journal ? 'page' : 'false'}"><span>${studyEsc(c)}</span><small>${getCompanyQuestions(c).length}</small></button>`).join('')}<div class="company-nav-tools"><button data-interview="new-company">＋ 新建公司面经</button><button data-interview="tab" data-tab="records" class="${journal ? 'active' : ''}">我的面试复盘 <small>${interviewRecords.length}</small></button></div></aside><div class="interview-company-main"><div class="interview-mobile-tools"><label>公司<select id="interviewCompany">${studyOptions([['all','全部公司'], ...companies.map(c=>[c,c])],interviewCompany)}</select></label><button class="study-secondary" data-interview="tab" data-tab="${journal ? 'sources' : 'records'}">${journal ? '返回公司问答' : '我的复盘'}</button></div>${journal ? interviewRecordsHTML() : `<header class="interview-company-head"><div><h2>${studyEsc(selected)}</h2></div><div class="interview-company-actions"><button class="study-primary" data-interview="add-company" data-company="${studyEsc(interviewCompany)}">＋ 添加面经题</button><button class="study-secondary" data-interview="company-practice" data-company="${studyEsc(interviewCompany)}" ${filtered.length ? '' : 'disabled'}>抽查这些题</button></div></header>${renderCompanyReferences()}<div class="interview-search-row"><input id="interviewSearch" type="search" aria-label="搜索公司面经问答" value="${studyEsc(interviewKeyword)}" placeholder="搜索这个公司的问题或答案"><button class="study-text" data-interview="reset-search">清空</button></div><div class="study-list-bar"><div class="study-list-heading"><h2>题目 <span>${filtered.length}</span></h2>${studyAnswerToggle(filtered)}</div>${filtered.length ? renderStudyPagination(filtered.length,'top') : ''}</div>${filtered.length ? filtered.slice(p.start,p.end).map(renderInterviewQuestion).join('') + renderStudyPagination(filtered.length,'bottom') : '<div class="study-empty"><h3>暂无匹配的公司问答</h3><p>可以清空搜索，或添加这个公司的实际问题与答案。</p></div>'}`}</div></section>`;
}
